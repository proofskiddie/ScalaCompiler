...
//    assert(true, "AST does not have correct type")    
  }
...
//    testSemanticAnalyzer(If(Lit(true),Lit(1),Lit(())), If(Lit(true).withType(BooleanType),Lit(1).withType(IntType),Lit(()).withType(UnitType)).withType(IntType), 0, 0)
      testSemanticAnalyzer(VarAssign("x", Lit(5)), VarAssign("x", Lit(5).withType(IntType)).withType(IntType), 0, 0)
 }
