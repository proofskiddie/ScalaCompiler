...
class Scanner(in: Reader[Char]) extends Reader[Tokens.Token] with Reporter {
  ...
  // List of keywords
  // TODO: Update this as keyword are added to our language
  val isKeyword    = Set("def", "if", "else", "val", "var", "while", "=>", "new", "Array")
...
class BaseParser(in: Scanner) extends Parser(in) {
  ...
  def parseCode : Exp = {
    ...
    val pos = in.getCurrentPos()
    if (in.hasNext)
      expected(s"EOF")
    LetRec(Nil, res).withPos(pos)
  }
...
class SyntacticSugarParser(in: Scanner) extends BaseParser(in) {
  ...
  override def parseExpression = {
    ...
    if (isNewLine(in.peek)) {
      in.next()
      val res1 = parseExpression
      Let(freshName(), UnknownType, res, res1).withPos(res.pos)
    } else
      res
  }
...
class FunctionParser(in: Scanner) extends SyntacticSugarParser(in) {
  ...
  override def parseType = (in.peek, in.peek1) match {
    case (Ident(x), Keyword("=>")) =>
      val tp1 = super.parseType
      accept("=>")
      val tp2 = parseType
      FunType(List(("", tp1)), tp2)
    case (Delim('('), _) =>
      accept('(')
      val tplist = parseList(parseType, ',', tok => tok match {
        case Delim(')') => false
        case EOF => false
        case _ => true
      })
   ...
   */
  def parseArg: Arg = in.peek match {
    case Ident(x) =>
      val pos = in.next().pos
      accept(':')
      val tp = parseType
  ...
  def parseFunction: Exp = {
    accept("def")
    val pos = in.getCurrentPos()
    val Ident(name) = in.next()

    accept('(')
    val arglist = parseList[Arg](parseArg, ',', tok => tok match {
      case Delim(')') => false
      case EOF => false
      case _ => true
    })
    accept(')')
    val rtp = if (in.peek == Delim(':')) {
                accept(':')
                parseType
              } else
                UnknownType
    ...
    FunDef(name, arglist, rtp, fbody).withPos(pos)
  }
   ...
   * TODO: complete the function
   */
  def parseProgram : Exp = in.peek match {
    case Keyword("def") =>
      val pos = in.getCurrentPos()
      val funlist = parseList[Exp](parseFunction, ';', tok => tok match {
        case Keyword("def") => true
        case _ => false
      })
      accept(';')
      LetRec(funlist, parseExpression).withPos(pos)
    case _ =>
      val pos = in.getCurrentPos()
      LetRec(Nil, parseExpression).withPos(pos)
  }
  ...
  def parseTight = in.peek match {
    ...
    case  _ =>
      ...
      while (Delim('(') == in.peek) {
        accept('(')
        val simplist = parseList[Exp](parseSimpleExpression, ',', tok => tok match {
          case Delim(')') => false
          case EOF => false
          case _ => true
        })
        accept(')')
        res = App(res, simplist)
      }
      res.withPos(pos)
  }
...
class ArrayParser(in: Scanner) extends FunctionParser(in) {
  ...
  override def parseTight = {
    val ret = super.parseTight
    val pos = ret.pos
    if (Delim('=') == in.peek) {
      ret match {
        case App(Ref(s), List(Ref(i))) =>
          accept('=')
          val simp = parseSimpleExpression
          Prim("block-set", List(Ref(s).withPos(pos), Ref(i).withPos(pos), simp)).withPos(pos).withType(UnitType)
        case App(Ref(s), List(Lit(i))) =>
          accept('=')
          val simp = parseSimpleExpression
          Prim("block-set", List(Ref(s).withPos(pos), Lit(i).withPos(pos), simp)).withPos(pos).withType(UnitType)
      }
    } else ret
  }
